  ...
  const server = createServer(async (request, response) => {
    ...
    try {
      ...
      if (pathname === "/api/local/jira-connection") {
        if ([...url.searchParams.keys()].length > 0) {
          throw new ApiError(400, "UNKNOWN_QUERY_PARAMETER", "Jira 連線介面不接受查詢引數");
        }
        if (request.method === "GET") {
      ...
      if (pathname === "/api/local/jira-connection/sync") {
        ...
        if ([...url.searchParams.keys()].length > 0) {
          throw new ApiError(400, "UNKNOWN_QUERY_PARAMETER", "Jira 同步介面不接受查詢引數");
        }
        ...
        if (request.method === "POST") {
          ...
          if (input.projectId === JIRA_PROJECT_ID) {
            throw new ApiError(
              ...
              "請在 Jira 中新建議題，Taskboard 當前只同步已分配給你的任務",
            );
          }
      ...
      if (taskRoute) {
        ...
        if (action === "archive" && request.method === "POST") {
          const current = database.getTask(id);
          if (current?.source === "jira") {
            throw new ApiError(409, "JIRA_ARCHIVE_UNAVAILABLE", "Jira 任務由同步範圍自動管理，不能手動歸檔");
          }
