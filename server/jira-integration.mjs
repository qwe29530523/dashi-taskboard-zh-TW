...
export function taskStatusFromJira(status) {
  ...
  if (category === "done") {
    return includesAny(name, ["cancel", "reject", "取消", "拒絕"]) ? "canceled" : "done";
  }
  if (category === "new") {
...
function jiraOriginId(manifest) {
  ...
  if (!applicationId) {
    throw new ApiError(502, "INVALID_JIRA_ORIGIN", "Jira 未返回穩定的例項身份");
  }
  ...
  async function request(config, pathname, init = {}) {
    ...
    if (response.status === 401 || response.status === 403) {
      throw new ApiError(
        401,
        "JIRA_AUTH_FAILED",
        "Jira 登入失敗，請檢查使用者名稱、密碼、Basic Auth 或 CAPTCHA 狀態",
      );
    }
    if (response.status >= 300 && response.status < 400) {
      throw new ApiError(400, "JIRA_REDIRECT", "Jira 地址發生重定向，請填寫最終訪問地址");
    }
    if (!response.ok) {
      throw new ApiError(
  ...
  async function resolveTransition(config, issueKey, targetStatus) {
    ...
    if (matches.length > 1) {
      throw new ApiError(
        ...
        `Jira 有多個工作流操作可將 ${issueKey} 移到目標狀態，請在 Jira 中選擇`,
        {
  ...
  return {
    ...
    async reconcile() {
      ...
      if (!config || config.version !== 2) {
        throw new ApiError(409, "JIRA_NOT_CONFIGURED", "Jira 尚未完成穩定身份配置");
      }
