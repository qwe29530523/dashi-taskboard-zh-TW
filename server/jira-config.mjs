...
export function normalizeJiraUrl(value) {
  if (typeof value !== "string" || value.includes("?") || value.includes("#")) {
    throw new JiraConfigError(
      "INVALID_JIRA_URL",
      "Jira 地址必須使用 http 或 https，且不能包含賬號、查詢引數或片段",
    );
  }
  let url;
  try {
...
function validateProjects(value) {
  ...
  if (!Array.isArray(value) || value.length > 20) {
    throw new JiraConfigError("INVALID_JIRA_PROJECTS", "Jira 專案必須是最多 20 項的陣列");
  }
  const projects = value.map((project) => {
    if (
      ...
      throw new JiraConfigError(
        ...
        "Jira 專案名稱或 Key 不能為空、不能包含控制字元，且不能超過 128 個字元",
      );
    }
    return project.trim();
  });
...
function parseConfig(value) {
  if (
    ...
    throw new JiraConfigError("INVALID_JIRA_CONFIG", "Jira 配置檔案無效");
  }
  ...
  if (Object.keys(value).some((key) => !allowedKeys.has(key))) {
    throw new JiraConfigError("INVALID_JIRA_CONFIG", "Jira 配置檔案包含未知欄位");
  }
  ...
  if (
    ...
    throw new JiraConfigError("INVALID_JIRA_CONFIG", "Jira 配置缺少穩定例項身份");
  }
  if (typeof value.displayName !== "string" || !value.displayName.trim()) {
    throw new JiraConfigError("INVALID_JIRA_CONFIG", "Jira 配置缺少使用者顯示名稱");
  }
  return {
